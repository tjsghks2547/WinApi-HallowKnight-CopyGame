...
{
	if (nullptr == CSoundMgr::GenInst()->GetSoundDevice())
...
bool CSound::LoadWaveSound(const wstring& _strPath)
{
	...
	if (FAILED(CSoundMgr::GenInst()->GetSoundDevice()->CreateSoundBuffer(&m_tBuffInfo, &m_pSoundBuffer, NULL)))
	{
...
{
	CSoundMgr::GenInst()->RegisterToBGM(this);
